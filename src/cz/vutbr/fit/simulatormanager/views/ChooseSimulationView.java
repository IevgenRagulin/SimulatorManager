...
public class ChooseSimulationView extends VerticalLayout implements View {
	...
	viewSimButton.addClickListener(new Button.ClickListener() {
		...
		if (isSelectedSimulationRunning()) {
		    navigator.navigateTo(SimulatormanagerUI.RUNNINGSIMULATIONS + "/" + getSimulatorList().getValue());
		} else {
		    navigator.navigateTo(SimulatormanagerUI.PASTSIMULATIONS + "/" + getSimulationList().getValue());
		}
	    }
	});
	...
	mainLayout.addComponent(new Label(
		"<p style='text-align: center; margin-left: 20px'><b>Select a simulator</b></p>", ContentMode.HTML));
	...
	simulationSessionsLabel = new Label(SIMULATIONS_ON_SELECTED_SIMULATOR, ContentMode.HTML);
	...
	bottomVerticalSplitPanel.setHeight("90%");
	...
	deleteButton.setDescription("Deletes data about chosen simulation from the database");
	actionButtons.addComponent(deleteButton);
	viewSimButton.setDescription("Look at the flight instruments of finsihed or currently running simulation");
	actionButtons.addComponent(viewSimButton);
	finishSimButton.setDescription("Can be applied to a running simulation. "
