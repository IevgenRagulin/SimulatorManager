...
public class FreeFormStatementDelegateSimulationsImpl implements
	...
	final static Logger LOG = LoggerFactory
			.getLogger(FreeFormStatementDelegateSimulationsImpl.class);
	...
	public int storeRow(Connection conn, RowItem row)
		...
		Integer simulationId = (Integer) row.getItemProperty(
				SimulationCols.simulationid.toString()).getValue();
		Boolean isSimOn = (Boolean) row.getItemProperty(
				SimulationCols.issimulationon.toString()).getValue();
		Boolean isPaused = (Boolean) row.getItemProperty(
				SimulationCols.issimulationpaused.toString()).getValue();
		String query = null;
		if (row.getId() instanceof TemporaryRowId) {
			throw new UnsupportedOperationException();
		} else {
			query = "UPDATE simulation SET issimulationon=?, issimulationpaused=?"
					+ "WHERE simulationid=?";
		}
