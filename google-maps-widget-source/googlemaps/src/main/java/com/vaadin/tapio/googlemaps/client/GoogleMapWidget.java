...
public class GoogleMapWidget extends FlowPanel implements RequiresResize {
	...
	private final static Logger logger = Logger.getLogger(GoogleMapWidget.class
			.getName());
	...
	private MarkerOptions createMarkerOptions(GoogleMapMarker googleMapMarker) {
		...
		if (googleMapMarker.getIconUrl() != null) {
			if ((googleMapMarker.getAnchX() != -1.0)
					&& (googleMapMarker.getAnchY() != -1.0)
					&& (googleMapMarker.getOrigX() != -1.0)
					&& (googleMapMarker.getOrigY() != -1.0)
					&& (googleMapMarker.getSizeX() != -1.0)
					&& (googleMapMarker.getSizeY() != -1.0)) {
				Point origin = Point.create(googleMapMarker.getOrigX(),
						googleMapMarker.getOrigY());
				Point anchor = Point.create(googleMapMarker.getAnchX(),
						googleMapMarker.getAnchY());
				Size size = Size.create(googleMapMarker.getSizeX(),
						googleMapMarker.getSizeY());
				MarkerImage markerImage = MarkerImage.create(
						googleMapMarker.getIconUrl(), size, origin, anchor); //
