...
public class FreeFormStatementDelegateEngineModels implements FreeformQueryDelegate {
	...
	return "SELECT * FROM enginemodel  WHERE simulatormodelid = " + simulatorModelId + "ORDER BY \"enginemodelorder\" ASC; ";
    }
	...
	LOG.info("Going to insert/update engine. Query: {}", query);
	...
	update.append(" WHERE enginemodelid = ").append(row.getItemProperty(EngineModelCols.enginemodelid.toString()).getValue());
    ...
    /**
     * Removes engine modelId from array
     * 
     * @param engineModelCols
     * @return
     */
    private EngineModelCols[] removeEngineModelIdFromArray(EngineModelCols[] engineModelCols) {
	return ArrayUtils.removeElement(engineModelCols, EngineModelCols.enginemodelid);
    }

    private String getInsertQuery(RowItem row) {
	StringBuffer insert = new StringBuffer("INSERT INTO enginemodel (");
	EngineModelCols[] engineModelCols = EngineModelCols.values();
	// don't insert engine model id, because it's autogenerated
	engineModelCols = removeEngineModelIdFromArray(engineModelCols);
	// column names
	for (EngineModelCols col : engineModelCols) {
	    insert.append(col.toString()).append(", ");
	}
	...
	// values
	for (EngineModelCols col : engineModelCols) {
	    appendInsertValue(insert, row, col.toString());
	}
	// remove last 2 characters
	insert.setLength(insert.length() - 2);
	insert.append(")");
